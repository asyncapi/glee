...
class Glee extends EventEmitter {
    ...
    send(message) {
        message.setOutbound();
        this._processMessage(this.router.getOutboundMiddlewares(), this.router.getOutboundErrorMiddlewares(), message);
    }
    /**
    ...
    injectMessage(message, serverName, connection) {
        ...
        this._processMessage(this.router.getMiddlewares(), this.router.getErrorMiddlewares(), message);
    }
    /**
    ...
    injectError(error, channel) {
        this._processError(this.router.getErrorMiddlewares(), error, { channel });
    }
    /**
    ...
    _processMessage(middlewares, errorMiddlewares, message) {
        const mws = middlewares
            ...
            msgForMiddleware.on('send', (m) => {
                m.inbound = false;
                m.outbound = true;
                this._processMessage(this.router.getOutboundMiddlewares(), this.router.getOutboundErrorMiddlewares(), m);
            });
