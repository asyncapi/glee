class GleeConnection {
    ...
    constructor({ connection, channels, serverName, server, parsedAsyncAPI }) {
        if (!connection)
            throw new Error('Please provide a connection object to create the connection representation.');
        if (!Array.isArray(channels) || !channels.length)
            throw new Error('Please provide a list of channels to create the connection.');
        if (!serverName)
            throw new Error('Please provide a server name to create the connection.');
        if (!server)
            throw new Error('Please provide a server object to create the connection.');
        if (!parsedAsyncAPI)
            throw new Error('Please provide a parsed AsyncAPI document to create the connection.');
