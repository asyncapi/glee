...
export default class WebSocketsServerAdapter extends GleeQuoreAdapter {
  ...
  private _handleBroadcastMessage(message: GleeQuoreMessage) {
  ...
  private _validateDirectMessage(message: GleeQuoreMessage) {
    if (!message.connection) {
      throw new Error('No WebSocket connection available for sending the message.')
    }
    if (!(message.connection instanceof GleeQuoreConnection)) {
      throw new Error('The connection object is not a valid GleeQuoreConnection instance.')
    }
  }

  private _handleDirectMessage(message: GleeQuoreMessage) {
    this._validateDirectMessage(message)
    message.connection.getRaw().send(message.payload)
  }

  private _send(message: GleeQuoreMessage) {
  ...
  private _createMessage(eventName: string, payload: any): GleeQuoreMessage {
