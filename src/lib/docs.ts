...
export default async (spec, config, resDir) => {
  ...
  const generator = new Generator(
    configData && configData.template ? configData.template : '@asyncapi/markdown-template',
    path.resolve(resDir ? resDir : './', configData && configData.folder ? configData.folder : 'docs'),
  )
  try {
