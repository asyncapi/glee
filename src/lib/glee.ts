...
export default class Glee extends EventEmitter {
  ...
  addAdapter(
    Adapter: typeof GleeAdapter,
    { serverName, server, parsedAsyncAPI }: { serverName: string; server: Server; parsedAsyncAPI: AsyncAPIDocument },
  ) {
  ...
  injectError(error: Error, channel?: string) {
    this._processError(this._router.getErrorMiddlewares(), error, new GleeMessage({ channel }))
  }
    ...
    const mws = middlewares
      ...
      .map((mw) => (msg: GleeMessage, next: MiddlewareCallback) => {
        ...
        msgForMiddleware.on('send', (m: GleeMessage) => {
          m.setOutbound()
          this._processMessage(this._router.getOutboundMiddlewares(), this._router.getOutboundErrorMiddlewares(), m)
        })
