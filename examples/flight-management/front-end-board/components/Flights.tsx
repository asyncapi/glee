...
export default function Flights({ airportCode }: any) {
  ...
  useEffect(() => {
    if (lastMessage !== null) {
      ...
      if (jsonMessage.name === 'allFlights') {
        ...
        setFlights(
          flights.map((flight) => {
            return flight.id === jsonMessage.flight.id ? jsonMessage.flight : flight
          }),
        )
      }
    }
  ...
  return (
    <section className=" bg-slate-800 text-white">
      ...
      <div className="flex flex-col divide-y-2 divide-solid font-mono">{departures.map(createFlight)}</div>
      <Header icon="/images/arrival.svg" text="Arrivals" />
      <div className="flex flex-col divide-y-2 divide-solid font-mono">{arrivals.map(createFlight)}</div>
    </section>
  )
}
